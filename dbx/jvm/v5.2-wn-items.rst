- Enables setting a client-side operations timeout (CSOT). This feature
  ...
  .. important::
    ...
    CSOT is an **experimental** feature and not yet production ready. This
    feature is optional in the current release and existing timeout options
    remain functional.

  If, and only if, the experimental CSOT feature is enabled, the following
  timeout options are ignored by the driver:
...
- Enables exact vector search by extending the ``VectorSearchOptions`` API
  ...
  To learn more about using the MongoDB Vector Search feature, see
  |vector-search-link|.
